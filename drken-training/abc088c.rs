...
fn main(){
  ...
  while let Some(point) = que.pop() {
    let cost = dist[point.0][point.1];
    for &(y,x) in &MOVES {
      let after = (point.0 as i32 + y, point.1 as i32 + x);
      if after.0 < 0 || after.1 < 0 || after.0 >= n  as i32 || after.1 >= m  as i32{continue;}
      let after = (after.0 as usize, after.1 as usize);
      if field[after.0][after.1]  == '.' && dist[after.0][after.1] > cost + 1 {
        dist[after.0][after.1] = cost + 1;
        que.push(after);
      }
    }
  }
  if dist[n-1][m-1] == std::i32::MAX {
    println!("-1");
    return;
  }
  let mini_cost = dist[n-1][m-1];
  let ans = white-mini_cost-1;
  println!("{}",ans);
}
