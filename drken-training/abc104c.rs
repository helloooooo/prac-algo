...
fn main(){
  ...
  let mut  min_v = pcn.iter().fold(0,|sum, pc| sum + pc.1 );
  for bit in 0..1 << d {
    let mut total = 0;
    let mut count = 0;
    for i in 0..d {
      if (bit & 1 << i) == 0 {continue;}
      total += 100 * (i + 1) * pcn[i as usize].0 + pcn[i as usize].1;
      count += pcn[i as usize].0;
    }
    if total >= g {
    ...
    for i in 0..d {
      if (bit & 1 << i) != 0 { continue; }
      let sub = 100 * (i + 1);
      let need = (g- total + sub -1) / sub;
      if need >= pcn[i as usize].0 { continue; }
      min_v = min(min_v, count + need);
    }
  }
  println!("{}",min_v);
}
