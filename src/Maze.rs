...
impl Points{
    fn bfs(&self,d:&mut Vec<Vec<i32>>,maze:&mut [[&str;10];10]) -> i32 {
        let mut v =  Vec::new();
        let mut init_d :Vec<Vec<i32>> = d.iter().map(|x| x.iter().map(|_y| INF ).collect()).collect();
        v.push(self.start);
        ...
        while !v.is_empty() {
            let v_pop = v.pop();
            if let Some(x) = v_pop {
                if x.0 == self.goal.0 && x.1 == self.goal.1 { break;}
                let vectors:Vec<_> = dx.iter().zip(dy.iter()).collect();
                for i in 0..4 {
                    let nx = x.0 + vectors[i].0;
                    let ny = x.1 + vectors[i].1;
                    if 0 <= nx && nx < 10  && 0 <= ny && ny < 10  && maze[nx as usize][ny as usize] != "#" && init_d[nx as usize][ny as usize] == INF {
                        v.push((nx,ny));
