...
fn main(){
  ...
  let mut v = (b'A'..b'Z').map(char::from).collect::<Vec<_>>();
  v.push('Z');
  for c in &x {
    let mut  index = v.iter().enumerate().filter(|s| s.1 == c).collect::<Vec<_>>().first().unwrap().0;
    if index + n > 25 {
      index = index+n-26;
    } else {
      index = index + n;
    ...
    print!("{}",v[index]);
    // print!("{}", std::char::from_digit(calc(c.to_digit(10).unwrap(), n) , 10).unwrap());
  }
}
fn calc(origin:u32,n:u32) -> u32 {
  if origin + n > 'Z'.to_digit(10).unwrap(){
    (origin + n) - 'Z'.to_digit(10).unwrap()
  } else {
    origin + n
  }
}
