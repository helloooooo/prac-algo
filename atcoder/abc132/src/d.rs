...
fn main(){
    ...
    let red = n-k;
    let mut fact = vec![Mod::new(1);8000];
    for i in 1..fact.len() {
        fact[i] = fact[i-1]*Mod::new(i as _);
    }
    let combination = |n:usize,red:usize| -> Mod{
        fact[n]/ (fact[red] * fact[n-red])
    };
    let sub = |n,r| combination(n+r-1,r);
    ...
    for j in 1..k+1 {
        ...
        let pos = combination(k-1,j-1);
        let fact = sub(j+1,red-sel);
        let ans = pos * fact;
        println!("{}",ans);
    }
}
